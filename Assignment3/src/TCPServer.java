...
class TCPserver { //opens a new class
	...
	public static void main(String argv[]) throws Exception { //opens the main of the class
		...
		String path = "C:/Users/Philip/OneDrive/Documents/New folder/index.txt";// Variable indicating the location of the text file
		
		FileReader fr = new FileReader(path); // variable fr for reading in information from the text file
		BufferedReader FileReader = new BufferedReader(fr); //reads each line of the text file
		
		int lines = 2; // variable for the amount of lines in the text file
		String[] FilePath = new String[lines]; // array for the the amount of lines in the text file
		
		for (int i = 0;i<lines; i++){ // loop for going reading each line of the text file
			FilePath[i] = FileReader.readLine(); // puts each line of the text file into a position in the array
		}
		...
		while (true) {// loop for code to run while the server is connected
			...
			if (InitialMessage.contains("server_is") == true){ // checks if the variable is server_1s and runs the if statement if it is
				...
				outToClienta.writeBytes( x + "\n");// outputs the string to the client
					...
					for(int i = 0; i< lines; i++){//loops through the array
					
					outToClientb.writeBytes(FilePath[i] + "\n"); // outputs the string to the the client
					}
					}
				
			
			
			if (InitialMessage.contains("server_put") == true){ // checks if the variable is server_put and runs the if statement if it is
				String x = "Add in the filename and path you want:"; // string variable containing message to be outputted
				
				outToClienta.writeBytes( x + "\n");// outputs the string to the client
				String NewFilename = inFromClientb.readLine();// reads in the new Filename and path sent in from the client
				...
				pr.println(NewFilename + " \n");// prints the new filename and path into the text file
				lines++; //increments the number of lines now in the text file
				String l = "Filename and path added successfully"; // string variable with message to be outputted
				outToClientb.writeBytes(l + "\n"); // outputs the string to the the client
				pr.close(); // closes the printwriter
			}
			
			if (InitialMessage.contains("server_get") == true) //Checks if the Variable is server_get and runs the if statement
			{
				String y = "Enter the Filename you'd like "; // string variable containing message to be outputted
				outToClienta.writeBytes(y + "\n");// outputs the string to the client
				String FileEntered = inFromClientb.readLine();// reads in the new name and number sent in from the client
			for (int j=0;j<lines; j++)
			{
				if (FilePath[j].contains(FileEntered) == true)// loops through the array storing the text file lines and checks if any of them match the name entered matches the
				{
					outToClientb.writeBytes(FilePath[j] + "\n"); // outputs the Filename and path entered
				}
			}
			}
			
			if (InitialMessage.contains("server_quit") == true){//Checks if the Variable is server_quit and runs the if statement 
				String close = "The connection has been terminated goodbye";// string variable containing message to be outputted
				outToClienta.writeBytes( close + "\n");// outputs the string to the client
